...
public class JSpinnerDateEditor extends JSpinner implements IDateEditor,
    ...
    public JSpinnerDateEditor() {
        ...
        noActive = Color.BLACK;
    }

    public String getText() {
        return ((JSpinner.DateEditor) getEditor()).getTextField().getText();
    }

    public void setText(String stringDate) {
        try {
            this.date = dateFormatter.parse(stringDate);
            setDate(date);
        } catch (ParseException ex) {
            System.err.println("Error stringDate invalid:" + ex);
        }
    }
    ...
    public void setForeground(Color noActivo, Color valido, Color error) {
        this.noActive = noActivo;
        ((JSpinner.DateEditor) getEditor()).getTextField().setForeground(noActivo);
    }
    ...
    public Color getNoActiveColor() {
        return noActive;
    }

    public void setUI(TextUI textUI) {
        ((JSpinner.DateEditor) getEditor()).getTextField().setUI(textUI);
    }


    public static void main(String[] args) {
        JFrame frame = new JFrame("JSpinnerDateEditor");
        JSpinnerDateEditor jsde = new JSpinnerDateEditor();
        jsde.setDate(new Date());

        frame.getContentPane().add(jsde);
        frame.pack();
        frame.setVisible(true);
        System.out.println(jsde.getText());
    }
